...
document
  ...
  .addEventListener("click", (e) => {
    ...
    if (e.target.nodeName == "A") {
      ...
      showPagingList(products.length); // 매개값을 설정해야 페이지 바꿀 때마다 유지됨.
      let start = 0,
        end = 0;
      start = (page - 1) * 5;
      end = page * 5;
      ...
      showProductList(products.slice(start, end)); // 위에 반복문과 동일.
    } // end of if.
  });
...
const product5 = products.slice(0, 5); //전체목록에서 5개 상품만 출력. 마지막
...
showPagingList(products.length); // 설정 안하면 초기값인 50을 기준으로 보여줌
